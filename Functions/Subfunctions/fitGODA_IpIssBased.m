function [Out,fun_Best,idx_Best,fval_Best] = fitGODA_IpIssBased(xIp,yIp,xIss,yIss,xIratio,yIratio,powera,powerb,TauO_Best,TauDA_Best,fun,OinfIfun_names,DAinfIfun_names,...
    Grectfun_names,options,plot_flag,display_flag,Il_valsp,Vm_valsp,Colors,type,inclIratio_flag,memorySave_flag,varargin)
...
for ifn=1:length(OinfIfun_names)
    ...
    Oton = @(p,X,t) yfunx(t,TauO_Best(X),OinfIfun(p(pOIidx),X(:,VarOinfI)),0);
    DAton = @(p,X,t) yfunx(t,TauDA_Best(X),DAinfIfun(p(pDAIidx),X(:,VarDAinfI)),1);

    iChR2On = @(p,X,t) Grectfun(p(pGidx),X(:,VarGrect)).*(Oton(p,X,t)).^powera.*(DAton(p,X,t)).^powerb;


    if inclIratio_flag
        %stablebecause otherwise order weights and targets incorrect
        %(probably unecessary when sort_flag on but to be sure)
        xT = unique([xIss;xIp;xIratio],'rows','stable');
        [~,idxss,~] = intersect(xT,xIss,'rows','stable');
        [~,idxp,~] = intersect(xT,xIp,'rows','stable');
        [~,idxratio,~] = intersect(xT,xIratio,'rows','stable');
        OBJ=@(p) objfunIpeakIssIratio(p,xT,idxss,idxp,idxratio,yIp,yIss,yIratio,iChR2On,options);
    else
    ...
    ITau = xIp(:,2);
    TauOoff = TauO_Best([xIp(:,1),zeros(size(xIp(:,2)))]);
    TauDAoff = TauDA_Best([xIp(:,1),zeros(size(xIp(:,2)))]);
    ...
    if plot_flag
        ...
        for ip=1:length(Il_valsp)
            idxx = find(xIp(:,2)==Il_valsp(ip));
            [xvals,idxvals] = sort(xIp(idxx,1));
            Iratio = IssMplot(idxx(idxvals))./IpeakMplot(idxx(idxvals));
            if length(xvals)>1
                plot(xvals,Iratio,type(ifn),'color',Colors(ip),'DisplayName',sprintf('fit %5.2e',Il_valsp(ip)))
                hold on
                plot(xIss(idxx,1),yIss(idxx)./yIp(idxx),'*','color',Colors(ip),'DisplayName',sprintf('data %5.2e',Il_valsp(ip)))
            end
        end
        hold off
        xlabel('Vm (mV)');
        ...
        for ip=1:length(Vm_valsp)
            ...
            Iratio = IssMplot(idxx(idxvals))./IpeakMplot(idxx(idxvals));
            if length(xvals)>1
                plot(xvals,Iratio,type(ifn),'color',Colors(ip),'DisplayName',sprintf('fit %5.2e',Vm_valsp(ip)))
                hold on
                plot(xIss(idxx,2),yIss(idxx)./yIp(idxx),'*','color',Colors(ip),'DisplayName',sprintf('data %5.2e',Vm_valsp(ip)))
            end
        end
        hold off
        xlabel('Il (W/m�)');
    ...
    function OBJ=objfunIpeakIss(p,tX,tPeak,tSS,IChR2Onfun,options)
        ...
        %
        %         for idebug = 1:size(IChR2On,2)
        %             figure(100)
        %             hold on
        %             plot(to',IChR2On(:,idebug))
        %         end
        %         hold off
    ...
    function OBJ=objfunIpeakIssIratio(p,tX,idxss,idxp,idxratio,tPeak,tSS,tIratio,IChR2Onfun,options)
        to=0:options.dt:options.tend;
        IChR2On=IChR2Onfun(p,tX,to);
        [~,idxM]=max(abs(IChR2On),[],2);
        idxM2=[[1:length(idxM)]',idxM];
        ...
        IssM=(IChR2On(:,end));
        IratioM =IssM./IpeakM;
        OBJ_M=[options.wpeak.*(IpeakM(idxp)-tPeak);options.wss.*(IssM(idxss)-tSS);options.wratio.*(IratioM(idxratio)-tIratio)];
        OBJ=sqrt(nanmean(OBJ_M(:).^2));
    end
end